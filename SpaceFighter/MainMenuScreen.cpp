...
void MainMenuScreen::LoadContent(ResourceManager& resourceManager)
{
	...
	Font *pFont = resourceManager.Load<Font>("Fonts\\Ethnocentric.ttf");//Originally Arial
	...
	for (int i = 0; i < COUNT; i++)
	{
		...
		pItem->SetFont(pFont);
		pItem->SetColor(Color::GRAY); // Originally White or Blue
	...
	GetMenuItem(START_GAME)->SetOnSelect([this](){
		SetOnRemove([this](){ AddScreen(new GameplayScreen(1)); });//Originally no index was passed into Gameplay screen constructor
		Exit();
	});
...
void MainMenuScreen::Update(const GameTime& gameTime)
{
	...
	for (MenuItem* pItem : GetMenuItems())
	{
		...
		pItem->SetColor(isSelected ? Color::GRAY : Color::RED); //Originally White then Blue - White for selected
